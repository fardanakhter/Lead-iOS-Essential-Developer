...
final class EssentialImageCommentsAPIEndToEndTests: XCTestCase {
    ...
    private var imageCommentsURL = {
        // JSON hosted on https://jsonbin.io for a fixed time period
        // Create another url when expired
        URL(string: "https://api.jsonbin.io/v3/qs/64d140638e4aa6225ecc4715")!
    }()
    ...
    private func getFeedResult(file: StaticString = #file, line: UInt = #line) -> Loader.Result? {
        ...
        let loader = Loader(url: imageCommentsURL, httpClient: client, mapper: ImageCommentMapper.map)
