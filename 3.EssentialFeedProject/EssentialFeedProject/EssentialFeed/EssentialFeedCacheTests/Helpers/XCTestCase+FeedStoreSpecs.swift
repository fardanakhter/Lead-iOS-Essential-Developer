...
extension FeedStoreSpecs where Self: XCTestCase {
    ...
    func assertThatRetrieveDeliversLastInsertedNonEmptyCache(on sut: FeedStore, file: StaticString = #file, line: UInt = #line) {
        ...
        expect(sut, toCompleteRetrievalWith: .success(CachedFeed(feed: feed, timestamp: timestamp)), file: file, line: line)
    }
    ...
    func assertThatRetrieveHasNoSideEffectOnNonEmptyCache(on sut: FeedStore, file: StaticString = #file, line: UInt = #line) {
        ...
        expect(sut, toCompleteRetrievalTwiceWith: .success(CachedFeed(feed: feed, timestamp: timestamp)), file: file, line: line)
    }
    ...
    func assertThatInsertionOverridesPreviouslyInsertedCache(on sut: FeedStore, file: StaticString = #file, line: UInt = #line) {
        ...
        expect(sut, toCompleteRetrievalWith: .success(CachedFeed(feed: secondCache.feed, timestamp: secondCache.timestamp)), file: file, line: line)
    }
    ...
    func expect(_ sut: FeedStore, toCompleteRetrievalWith expectedResult: FeedStore.LoadResult, file: StaticString = #file, line: UInt = #line) {
        ...
        sut.loadFeedCache { result in
            ...
            case let (.success(receivedCachedFeed), .success(expectedCachedFeed)):
                XCTAssertEqual(receivedCachedFeed?.feed, expectedCachedFeed?.feed)
                XCTAssertEqual(receivedCachedFeed?.timestamp, expectedCachedFeed?.timestamp)
    ...
    func insert(_ cache: (feed: [LocalFeedImage], timestamp: Date), to sut: FeedStore) -> Error? {
        ...
        sut.insertFeedCache(with: cache.feed, and: cache.timestamp) { insertResult in
            if case let .failure(insertError) = insertResult {
    ...
    func delete(from sut: FeedStore) -> Error? {
        ...
        var receivedError: Error?
        sut.deleteFeedCache { deleteResult in
            if case let .failure(deleteError) = deleteResult {
                receivedError = deleteError
            }
            exp.fulfill()
        }
